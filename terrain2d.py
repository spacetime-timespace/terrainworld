...
class Terrainer(arcade.Window):
    def __init__(self,WIDTH=240, HEIGHT=180, FPS=60, PIX = 256, WSIZE = 32, SPEED=1,seed=s5,MSPEED=16):
    ...
    def on_update(self,delta):
        for i in range(int(self.pos[0]//self.p)-1,int(self.pos[0]//self.p)+2):
            for j in range(int(self.pos[1]//self.p-1),int(self.pos[1]//self.p)+2):
                if (i,j) not in self.ch:
                    print("LOADING CHUNK: "+str((i,j)))
                    self.ch.append((i,j))
                    for k in range(self.p+1):
                        for l in range(self.p+1):
                            self.grid[(i*self.p+k,j*self.p+l)] = self.n(((i*self.p+k)/self.z,(j*self.p+l)/self.z))
        ...
        if self.cmouse[2]!=0:
            try:
                k = self.cmouse[0]
                l = self.cmouse[1]
                if self.cmouse[2] == 1 and self.grid[(k,l)]>-1:
                    self.grid[(k,l)] -= delta*self.sp
                if self.cmouse[2] == 2 and self.grid[(k,l)]<1:
                    self.grid[(k,l)] += delta*self.sp
                print(self.grid[(k,l)])
