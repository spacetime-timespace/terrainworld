...
class Terrainer(arcade.Window):
    def __init__(self,WIDTH=240, HEIGHT=180, FPS=60, PIX = 256, WSIZE = 32, SPEED=1):
    ...
    def setup(self):
        self.grid = [[s5([i/self.w,j/self.w]) for j in range(self.p)] for i in range(self.p)]
...
def main():
    window = Terrainer(PIX=64,WSIZE=16)
    window.setup()
    arcade.run()
